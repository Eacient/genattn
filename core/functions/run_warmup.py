...
from core.functions.engine_warmup import train_one_epoch
...
def get_args():
    ...
    parser.add_argument('--approx_prior', action='store_true')
    parser.set_defaults(approx_prior=True)
    parser.add_argument('--warmup_cam_net', action='store_true')
    parser.add_argument('--warmup_gvae', action='store_true')
    parser.set_defaults(warmup_gvae=True)
...
def get_model(args, **kwargs):
    ...
    model = create_model(
        ...
        **kwargs,
        ...
        use_wta=args.use_wta,
        approx_prior=args.approx_prior,
        # bg_thresh=args.bg_thresh,
        cam_net_pretrained=args.cam_net_pretrained,
        warmup_cam_net=args.warmup_cam_net,
        warmup_gvae=args.warmup_gvae
    )
    return model
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch( #gen epoch
            ...
            debug=args.debug, amp=args.amp, accum_iter=args.accum_iter
        )
        if args.output_dir:
